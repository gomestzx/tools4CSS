...
const NavbarTailwindComponentsPage = () => {
    ...
    const { isFavorited, handleFavorite } = useFavoriteTool("Navbar Tailwind Components");
    ...
    const title = Array.isArray(slug) ? slug[0] : (`${slug?.charAt(0).toUpperCase()}${slug?.slice(1)}`);
        ...
        <>
            ...
            <div className="w-full lg:w-5/6 mx-auto font-medium">
                <div className="flex flex-row justify-between items-center mx-4 lg:mx-0 cap">
                    <Breadcrumb
                        links={[{ href: "/", label: "Home" }, { href: "/", label: "Tailwind Components" }, { label: title }]}
                    />
                    <FavoriteButton
